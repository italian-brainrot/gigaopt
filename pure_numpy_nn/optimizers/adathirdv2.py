...
class AdaThirdV2(Optimizer):
    """
    An experimental third-order optimizer with momentum on the third moment.
    """
    def __init__(self, lr=0.001, beta1=0.9, beta2=0.999, beta3=0.9, epsilon=1e-8):
        ...
        self.v = None
        self.c = None
        self.c_v = None # Velocity for the third moment
    ...
    def step(self, params, grads):
        ...
        if self.m is None:
            ...
            self.c_v = [np.zeros_like(p) for p in params]
        ...
        for i, (param, grad) in enumerate(zip(params, grads)):
            ...
            # Update biased third moment estimate with momentum
            self.c_v[i] = self.beta3 * self.c_v[i] + (1 - self.beta3) * grad_diff
            self.c[i] = self.beta3 * self.c[i] + (1 - self.beta3) * (self.c_v[i] ** 2)
